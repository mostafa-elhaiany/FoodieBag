...
public struct ViscosityCanvasMap: View {
    ...
    public var body: some View {
        ZStack {
            viscosityView2(color: Color(rgb: 0xD2D90B), scale: $scale1)
            viscosityView2(color: Color(rgb: 0xF2EB79), scale: $scale2)
                .blendMode(.screen)
    ...
    private func viscosityView2(color: Color, scale: Binding<CGFloat>) -> some View {
        GeometryReader { geo in
            if isFillMode {
                ViscosityCanvas2(color: color) {
                    circle(cnavasSize: geo.size, scale: scale)
                }
            } else {
                ViscosityCanvas2(color: color, thresholdMin: 0.5, thresholdMax: 0.7) {
                    circle(cnavasSize: geo.size, scale: scale)
                }
            }
        }
...
fileprivate
struct ViscosityCanvas2<Symbols: View> : View {
...
struct ViscosityCanvas_Previews: PreviewProvider {
